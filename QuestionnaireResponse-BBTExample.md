# BBTExample - v0.1.0

* [**Table of Contents**](toc.md)
* [**Artifacts Summary**](artifacts.md)
* **BBTExample**

## Example QuestionnaireResponse: BBTExample

Profile: [BBTQR](StructureDefinition-bbt-questionnaire-response.md)

* [LinkID](https://hl7.org/fhir/R4/formats.html#table): BBTExample
  * [Text](https://hl7.org/fhir/R4/formats.html#table): 
  * [Definition](https://hl7.org/fhir/R4/formats.html#table): 
  * [Answer](https://hl7.org/fhir/R4/formats.html#table)![](data:image/png;base64,): Questionnaire:[BBT評估量表](Questionnaire-BBTQuestionnaire.md)
* [LinkID](https://hl7.org/fhir/R4/formats.html#table): ![](icon-q-string.png)left-hand
  * [Text](https://hl7.org/fhir/R4/formats.html#table): 
  * [Definition](https://hl7.org/fhir/R4/formats.html#table): 
  * [Answer](https://hl7.org/fhir/R4/formats.html#table)![](data:image/png;base64,): 31
* [LinkID](https://hl7.org/fhir/R4/formats.html#table): ![](icon-q-string.png)right-hand
  * [Text](https://hl7.org/fhir/R4/formats.html#table): 
  * [Definition](https://hl7.org/fhir/R4/formats.html#table): 
  * [Answer](https://hl7.org/fhir/R4/formats.html#table)![](data:image/png;base64,): 63
* [LinkID](https://hl7.org/fhir/R4/formats.html#table): ![](data:image/png;base64,)Documentation for this format



## Resource Content

```json
{
  "resourceType" : "QuestionnaireResponse",
  "id" : "BBTExample",
  "meta" : {
    "profile" : [
      "http://example.org/StructureDefinition/bbt-questionnaire-response"
    ]
  },
  "questionnaire" : "http://example.org/Questionnaire/BBTQuestionnaire",
  "status" : "completed",
  "item" : [
    {
      "linkId" : "left-hand",
      "answer" : [
        {
          "valueInteger" : 31
        }
      ]
    },
    {
      "linkId" : "right-hand",
      "answer" : [
        {
          "valueInteger" : 63
        }
      ]
    }
  ]
}

```
